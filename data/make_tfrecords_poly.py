...
def extract_polyphony_and_release(midi_data, total_frames):
    """Asigna notas a canales independientes y aplica el Note Release."""
    ...
    # Registro de cuándo queda libre cada voz (incluyendo el tiempo de Release)
    voice_free_frame = np.zeros(MAX_VOICES, dtype=int)
    
    # Extraemos todas las notas y las ordenamos cronológicamente
    ...
    for note in notes:
        ...
        # EL TRUCO NOTE RELEASE: La "memoria" del pitch se extiende 1 segundo más
        release_f = min(total_frames, int((note.end + T_RELEASE_SEC) * FRAME_RATE))
        
        # 1. Buscar un canal/voz que esté libre
        assigned_voice = -1
        ...
        # 2. Si hay hueco, escribimos las curvas
        if assigned_voice != -1:
            # El pitch sobrevive hasta el release_f
            pitch_curves[assigned_voice, start_f:release_f] = note.pitch
            # La velocity se apaga en el end_f natural
            vel_curves[assigned_voice, start_f:end_f] = note.velocity / 127.0
            
            # Bloqueamos este canal hasta que termine su release
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = r"C:\Users\franc\Desktop\proyectos\DDSP\dataset\tfrecords_poly"
