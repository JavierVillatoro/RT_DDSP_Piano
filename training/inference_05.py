...
def synthesize_midi(midi_file, weights_folder, output_wav):
    ...
    dummy_inputs = {'pitches': dummy_pitch, 'velocities': dummy_vel, 'pedal': dummy_pedal}
    
    _ = model(dummy_inputs, training=False) # <--- ¡AHORA SÍ!
    ...
    print("[INFO] Sintetizando audio por trozos (Chunking)...")
    
    # --- NUEVO SISTEMA DE PROCESAMIENTO POR TROZOS ---
    chunk_frames = 500  # Trozos de 10 segundos (250 frames por segundo * 10) antes 2500
    audio_chunks = []
    
    # tqdm crea la barra de progreso que querías ver
    for start in tqdm(range(0, total_frames, chunk_frames), desc="Generando audio"):
        end = min(start + chunk_frames, total_frames)
        
        # Extraemos el "corte" de tiempo para cada tensor
        chunk_inputs = {
            'pitches': p_pitch[:, :, start:end, :],
            'velocities': p_vel[:, :, start:end, :],
            'pedal': p_pedal[:, start:end, :]
        }
        
        # Procesamos solo ese trocito
        chunk_audio = model(chunk_inputs, training=False)
        audio_chunks.append(tf.squeeze(chunk_audio).numpy())
        
    # Pegamos todos los trozos de audio uno detrás de otro
