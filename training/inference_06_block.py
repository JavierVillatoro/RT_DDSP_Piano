...
# ==============================================================================
# 4. POLYPHONIC WRAPPER (Orquestador Inferencia con Anti-Amnesia)
# ==============================================================================
class PolyphonicDDSPPianoDynamic(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        total_frames = tf.shape(pitches)[2]
        # El tamaño exacto con el que entrenó la red (3 segundos a 250Hz)
        CHUNK_FRAMES = 750 

        # --- CONTEXTO GLOBAL (En bloques para evitar Amnesia) ---
        ...
        for start in range(0, total_frames, CHUNK_FRAMES):
            end = tf.math.minimum(start + CHUNK_FRAMES, total_frames)
            c_t_chunk = self.context_net(context_input[:, start:end, :])
            c_t_list.append(c_t_chunk)
        c_t = tf.concat(c_t_list, axis=1) 
        ...
        for i in tqdm(range(self.n_voices), desc="Sintetizando Voces", unit="voz"):
            ...
            # --- CEREBRO GRU (En bloques para evitar Amnesia) ---
            amp_list, harm_list, noise_list, fk_list = [], [], [], []
            
            for start in range(0, total_frames, CHUNK_FRAMES):
                end = tf.math.minimum(start + CHUNK_FRAMES, total_frames)
                
                params_chunk = self.core(core_input[:, start:end, :], v_pitch[:, start:end, :])
                
                amp_list.append(params_chunk['amplitude'])
                harm_list.append(params_chunk['harmonics'])
                noise_list.append(params_chunk['noise_magnitudes'])
                fk_list.append(params_chunk['f_k'])
            
            # Unimos la partitura de parámetros (Canción completa)
            full_amp = tf.concat(amp_list, axis=1)
            full_harm = tf.concat(harm_list, axis=1)
            full_noise = tf.concat(noise_list, axis=1)
            full_fk_1 = tf.concat(fk_list, axis=1)
            
            # El Detuner no tiene memoria, procesa todo de golpe sin problema
            delta_f = self.detuner(v_pitch)
            full_fk_2 = full_fk_1 * tf.pow(2.0, (delta_f / 12.0))
            
            # --- SINTETIZADORES (De una sola pasada para mantener la fase intacta) ---
            audio_voice = self.harmonic_synth(full_fk_1, full_amp, full_harm)
            audio_voice += self.harmonic_synth(full_fk_2, full_amp, full_harm)
            audio_voice += self.noise_synth(full_noise)
...
if __name__ == "__main__":
    ...
    SALIDA_WAV = os.path.join(BASE_DIR, "resultado_ddsp_fase2_Mompou_block.wav")
