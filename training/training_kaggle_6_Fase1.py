...
RESUME_TRAINING = False  # Ponlo en True cuando quieras cargar pesos
PRETRAINED_DIR = '/kaggle/input/mis-pesos-fase1/'
...
class ContextNetwork(tf.keras.Model): 
    def __init__(self, context_dim=32, gru_units=64):
        super().__init__()
        # La entrada ahora recibirá más información, la capa Dense se encargará de comprimirla
    ...
    def call(self, inputs):
        # 'inputs' será un tensor que concatena Pedal, y un resumen de Pitches y Velocities
...
class TrainableReverb(tf.keras.Model): # <--- CAMBIO AQUÍ
    def __init__(self, ir_length=24000, lambda_ir=1e-2, **kwargs): #lambda_ir=1e-4
...
def run_training():
    ...
    with strategy.scope():
        ...
        if RESUME_TRAINING:
            print(f"\n[INFO] Activando RESUME_TRAINING. Buscando pesos en: {PRETRAINED_DIR}")
            try:
                ...
                print("[ÉXITO] ¡Pesos cargados correctamente! Continuando entrenamiento.")
            except Exception as e:
                print(f"[ERROR] No se pudieron cargar los pesos: {e}")
                print("Continuando el entrenamiento desde cero por seguridad...")
