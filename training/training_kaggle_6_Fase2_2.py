# ==============================================================================
# FASE 2 - FINE TUNING FÍSICO (INHARMONICITY & DETUNER ON) DETUNER IMPROVE %5 AND INITIALIZE KERNEL BIAS
# ==============================================================================
...
class Detuner(tf.keras.Model):
    def __init__(self):
        super().__init__()
        # 1. ORTODOXIA: Inicialización estricta a cero en Kernel y Bias
        self.dense = tf.keras.layers.Dense(
            1, 
            kernel_initializer='zeros', 
            bias_initializer='zeros'
        )
        self.trainable = True # FASE 2

    def build(self, input_shape):
        # ORTODOXIA: Desafinación estática también a cero absoluto
    ...
    def call(self, pitch_midi):
        ...
        # 2. INGENIERÍA: Restricción física (Hard-scaling a ±0.05 semitonos)
        dynamic_detune = tf.nn.tanh(self.dense(x)) * 0.05
        static_detune = tf.nn.tanh(self.static_detune) * 0.05
...
class InharmonicityModel(tf.keras.layers.Layer):
    # Devolvemos el l1_penalty a 0.1 (Respetando el paper)
    ...
    def call(self, pitch_midi):
        ...
        self.add_loss(loss_l1/8.0) #Division entre 8.0, Antes cobrando L1 PENALTY a la red 8 veces por cada paso de entrenamiento! 
